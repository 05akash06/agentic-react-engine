...
Available Tools:
1. web_search(query: str): Search web for real information. Keep queries simple (2-5 keywords).
2. calculate(expression: str): Solve math problems.
3. save_to_file(content: str): Save final plan or results to a file.

Formatting Rules:
- DO NOT wrap plain text sentences inside dollar signs ($).
- For currency, write prices clearly like "15 USD", "300 USD", or "$15" without wrapping full text phrases in $...$.
...
max_steps = st.sidebar.slider("Max Execution Steps", min_value=3, max_value=12, value=6)
temperature = st.sidebar.slider("Temperature", min_value=0.0, max_value=1.0, value=0.2, step=0.05)

if st.sidebar.button("🔄 Clear & Reset Engine"):
    st.rerun()
        ...
        while step <= max_steps:
            with st.expander(f"Step {step}", expanded=True):
                ...
                if action in tools:
                    ...
                    # Unique widget key prevents StreamlitDuplicateElementId error across steps
                    st.text_area("👁️ Observation", tool_result, height=100, key=f"obs_text_{step}")
